...
- No third-party component library or JS framework beyond Alpine + anime.js. Two focused, vendored, self-hosted utilities are allowed by exception, not as a general framework license:
  ...
  - Howler.js (`internal/assets/static/js/vendor/howler.min.js`, the two audio surfaces only), added deliberately for the per-question audio playback (#1088): its Web Audio AudioContext unlocks on the first user gesture, letting the engine preload every clip up front and play each question without a fresh tap -- fixing the iOS "audio goes silent on later questions" bug the old `<audio>` element had. Reach for it only for that audio playback.
