...
- Both trees are bundled: the player client (source `frontend/client/`, entries `app.js` / `join.js`, output `internal/client/static/js/dist/`) and the web/host tree (source `frontend/web/`, entries `host-bigscreen.js` / `share.js` plus the combined `admin.js` bundle, output `internal/assets/static/js/dist/`). `admin.js` folds the small admin/auth/home page modules (`cooldown.js`, `copy-prompt.js`, `password-length.js`, `quiz-reorder.js`, `quiz-image-upload.js`, `quiz-audio-upload.js`, `home.js`) into one bundle the layouts load (#1071); `host-bigscreen.js` and `share.js` stay separate because they are heavier and page-specific. Cross-tree shared modules live in `frontend/shared/` and are inlined into each tree's bundles via the `@shared/` import alias. The served `internal/*/static/js/` trees hold only built bundles (`dist/`) and vendored libs (`vendor/`, `htmx.min.js`) -- no un-minified source.
...
Every app-JS source file under `frontend/` is a real ES module: it uses `import` / `export`, never a `(function () { ... })()` IIFE wrapper or a bare classic script. The combined `admin.js` entry imports the small admin/auth/home page modules (`cooldown.js`, `copy-prompt.js`, `password-length.js`, `quiz-reorder.js`, `quiz-image-upload.js`, `quiz-audio-upload.js`, `home.js`) so their setup runs from one loaded module; it loads via `<script type="module">`, which already gives module scope, so no wrapper is needed. Each imported module guards its own init (it queries for its anchor element, runs through the shared `onDomReady` helper, or registers an `alpine:init` handler) rather than an inline `DOMContentLoaded` block, so the combined bundle no-ops for any feature whose anchor is absent on the current page.
